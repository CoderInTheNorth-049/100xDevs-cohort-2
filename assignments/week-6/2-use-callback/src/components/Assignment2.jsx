...
export function Assignment2() {
  // State to manage the input field's text 
  const [inputText, setInputText] = useState('');

  // Memoized event handler for displaying the alert
  const showAlert = useCallback(() => {
    alert(inputText);
  }, [inputText]); // Recalculate only when inputText changes

  return (
    <div>
      {/* Input field to capture user entered text */}
      <input
        type="text"
        value={inputText}
      ...
      {/* Pass the memoized showAlert function to the Alert component */}
...
const Alert = ({ showAlert }) => {
  // Button that triggers the memoized alert function
